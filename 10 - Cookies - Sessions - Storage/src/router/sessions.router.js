...
router.post('/login', async(req, res) =>{
    ...
    if(!email || !password){
        return res.redirect('/login?error=Debes completar todos los campos.')
    }

    let exReg = /^[-\w.%+]{1,64}@(?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63}$/i
    let valid = exReg.test(email)
    if(valid === false){
        return res.redirect('/login?error=Debes enviar un email valido.')
    }
    ...
    if(!existUser.success){
        return res.redirect(`/login?error=${existUser.error}`)
    }


    req.session.user = {
        name: existUser.existUser.name, email: email, rol: existUser.existUser.rol
    }
...
router.get('/logout', auth, async (req,res)=>{
    req.session.destroy(error =>{
        if(error){
            ...
            return res.status(500).json({InternalError: error});
        }
    })
